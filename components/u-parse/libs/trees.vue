<template>
	<view :class="'interlayer '+(c||'')" :style="s">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			...
			<!--音频-->
			<!--#ifdef MP-WEIXIN -->
			<view class="audio_box"  v-else-if="n.name=='audio'" :ref="n.attrs.id" :id='n.attrs.id'>
				<!-- {{n.attrs.source[ctrl[i]||0]}} -->
				<view class="left" @click.stop="onAudio(n,n.attrs.id,n.attrs.source[ctrl[i]||0])">
					<image class="image"  src="../../../pageB/static/icon/audio_01.png" v-if='is_audio && audio_id == n.attrs.id ' mode="aspectFit"></image>
					<image class="image" src="../../../pageB/static/icon/audio_02.png"  v-else mode="aspectFit"></image>
				</view>
				<view class="right">
				</view>
				<!-- {{n.attrs.id}}-{{n.attrs.author}}-{{n.attrs.autoplay}}-{{n.attrs.controls}}-{{n.attrs.loop}} -{{n.attrs.poster}}-{{n.attrs.source[ctrl[i]||0]}} -->
			</view>
			<!--#endif-->
			<!--#ifndef  MP-WEIXIN-->
			...
			<view v-else-if="n.name=='a'" :id="n.attrs.id" :class="'_a '+(n.attrs.class||'')" hover-class="_hover" :style="n.attrs.style"
			 :data-attrs="n.attrs" @tap.stop="linkpress">
				<trees class="_span" c="_span" :nodes="n.children" :isPreview="isPreview"/>
			</view>
			<!--广告-->
			...
			<view v-else-if="n.name=='li'" :id="n.attrs.id" :class="n.attrs.class" :style="(n.attrs.style||'')+';display:flex;flex-direction:row'">
				...
				<trees class="_li" c="_li" :nodes="n.children" :lazyLoad="lazyLoad" :loading="loading" :isPreview="isPreview"/>
			</view>
			<!--表格-->
			<view v-else-if="n.name=='table'&&n.c&&n.flag" :id="n.attrs.id" :class="n.attrs.class" :style="(n.attrs.style||'')+';display:grid'">
				<trees v-for="(cell,n) in n.children" v-bind:key="n" :class="cell.attrs.class" :c="cell.attrs.class" :style="cell.attrs.style"
				 :s="cell.attrs.style" :nodes="cell.children" :isPreview="isPreview"/>
			</view>
			<view v-else-if="n.name=='table'&&n.c" :id="n.attrs.id" :class="n.attrs.class" :style="(n.attrs.style||'')+';display:table'">
				<view v-for="(tbody, o) in n.children" v-bind:key="o" :class="tbody.attrs.class" :style="(tbody.attrs.style||'')+(tbody.name[0]=='t'?';display:table-'+(tbody.name=='tr'?'row':'row-group'):'')">
					<view v-for="(tr, p) in tbody.children" v-bind:key="p" :class="tr.attrs.class" :style="(tr.attrs.style||'')+(tr.name[0]=='t'?';display:table-'+(tr.name=='tr'?'row':'cell'):'')">
						<trees v-if="tr.name=='td'" :nodes="tr.children" :isPreview="isPreview"/>
						<trees v-else v-for="(td, q) in tr.children" v-bind:key="q" :class="td.attrs.class" :c="td.attrs.class" :style="(td.attrs.style||'')+(td.name[0]=='t'?';display:table-'+(td.name=='tr'?'row':'cell'):'')"
						 :s="(td.attrs.style||'')+(td.name[0]=='t'?';display:table-'+(td.name=='tr'?'row':'cell'):'')" :nodes="td.children" :isPreview="isPreview"/>
					</view>
				</view>
			</view>
			<!--#ifdef APP-PLUS-->
			...
			<rich-text v-else-if="handler.use(n)" :id="n.attrs.id" :class="'_p __'+n.name" :nodes="[n]" class="rich"/>
			<!--#endif-->
			<!--#ifndef MP-WEIXIN || MP-QQ || APP-PLUS-->
			<rich-text v-else-if="!n.c" :id="n.attrs.id" :nodes="[n]" style="display:inline"  class="rich"/>
			<!--#endif-->
			<trees v-else :class="(n.attrs.id||'')+' _'+n.name+' '+(n.attrs.class||'')" :c="(n.attrs.id||'')+' _'+n.name+' '+(n.attrs.class||'')"
			 :style="n.attrs.style" :s="n.attrs.style" :nodes="n.children" :lazyLoad="lazyLoad" :loading="loading" :isPreview="isPreview"/>
		</block>
	</view>
</template>

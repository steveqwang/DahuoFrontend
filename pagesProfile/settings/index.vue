...
<!-- 
					<dz-link style="margin-bottom: 20rpx;" v-for="(item, index) in links" :key="index" icon="" :title="item.title"></dz-link> -->
					<dz-drop-links :list="links"></dz-drop-links>
					
					<view class="no-link">
						<text class="link-text" v-if="links.length<=0">还没有添加名片链接哦</text> <!-- 添加灰色小字 -->
						<text class="link-text" v-else>长按链接拖动改变顺序</text> <!-- 添加灰色小字 -->
			...
			<view class="section">
				...
				<view class="social-media-list" >
					<view class="social-media-item" v-for="(item, index) in socialMediaList" :key="index" @click="toMidea()">
	...
	export default {
			...
			return {
				...
				links: [{
					...
					},
					{
						type: 2,
						title: "图片描述",
						linkUrl: "https://www.baidu.com/1.jpg"
					}
				],
				...
				socialMediaList: [{
						name: '大众点评',
						icon: '../../static/icon/dianping_round.png',
						enabled: false
					},
					{
						name: '小红书',
						icon: '../../static/icon/xhs_round.png',
						enabled: false
					},
					{
						name: '微博',
						icon: '../../static/icon/weibo_round.png',
						enabled: false
					},
					{
						name: '抖音',
						icon: '../../static/icon/douyin_round.png',
						enabled: false
					}
				]
			};
		...
		methods: {
			...
			addLink() {
				// 跳转到添加链接页面逻辑
				uni.navigateTo({
					url: '../../pagesProfile/addLink/index'
				})
			},
			toggleSwitch(index) {
			...
			toMidea(){
				console.log(12)
				uni.navigateTo({
					url: '../../pagesProfile/editSocialMedia/index'
				})
			}
		}
	};
